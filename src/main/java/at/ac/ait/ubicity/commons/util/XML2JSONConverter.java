...
public final class XML2JSONConverter {
	...
	public final static void main(String... args) throws IOException {
		...
		// default behaviour: do not write JSON output files to local file
		// system
		boolean __shouldWriteJSON = false;
		if (args.length == 3)
			__shouldWriteJSON = args[2].toLowerCase().equals("dojsonwrite");
		...
		if (in.isDirectory()) {
			...
			System.out.println("processing xml file of " + in.length()
					+ " bytes took " + _lapse + " microseconds");
		}
		System.exit(0);
	}

	private void handleDirectory(File in, File outputDirectory) {
		final FileFilter _xmlFilter = (File _f) -> _f.getName().toLowerCase()
		...
		final Disruptor<XMLFile> disruptor = new Disruptor<XMLFile>(
				FileHandler.EVENT_FACTORY, (int) Math.pow(TWO, 17), exec);

		final EventHandler<XMLFile> _eventHandler = new FileHandler(
				outputDirectory, esclient);
		disruptor.handleEventsWith(_eventHandler);
		RingBuffer<XMLFile> ringBuffer = disruptor.start();

		CreateIndexRequestBuilder createIndexRequestBuilder = esclient.admin()
				.indices().prepareCreate("story");
		createIndexRequestBuilder.execute().actionGet();

		int fileCounter = 0;

		System.out.println("[INFO] beginning processing of XML files in "
				+ in.getAbsolutePath());
		long _start, _lapse;
		_start = System.nanoTime();
		...
		disruptor.shutdown();
		_lapse = (System.nanoTime() - _start) / 1000;
		System.out.println("[INFO] processed " + fileCounter + " files in "
				+ _lapse + " microseconds");
	}
}
	...
	public void convert(File _f) throws IOException {
		...
		try {
			...
			Logger.getLogger(this.getClass().getName()).fine(
					"caught a JSONException : " + somethingWrong.toString());
		}

		if (XML2JSONConverter.mustWriteJSON) {
			String __JSONfileName = outputDirectory + "/" + __id
					+ XML2JSONConverter.JSON_EXTENSION;
		...
		/**
		 * String __JSONfileName = outputDirectory + "/" + __id +
		 * XML2JSONConverter.JSON_EXTENSION; try (FileWriter _jsonFile = new
		 * FileWriter( new File( __JSONfileName ) )) { _jsonFile.write(
		 * json.toString( 3 ) ); _jsonFile.flush(); _jsonFile.close(); }
		 */
